...
//             self::initSessions();
    ...
    static function isAjax() {
    	return ( ! empty( $_SERVER[ 'HTTP_X_REQUESTED_WITH' ] ) && strtolower( $_SERVER[ 'HTTP_X_REQUESTED_WITH' ]) == 'xmlhttprequest'
    		AND strpos(filter_input(INPUT_SERVER, 'HTTP_REFERER'), admin_url('customize.php')) === false);
    }
...
//             add_submenu_page(apply_filters('CMA_admin_parent_menu', 'options-general.php'), CMA_Settings::__('User Guide'), CMA_Settings::__('User Guide'), 'manage_options', self::ADMIN_USER_GUIDE, array(get_class(), 'displayAboutPage'));
    ...
    public static function displaySettingsPage()
    {
        ...
        wp_register_script('cmdm-settings-search', CMA_URL . '/views/resources/js/settings-search.js', array('jquery'));
        wp_enqueue_script('cmdm-settings-search');
        wp_enqueue_style('cmdm-settings', CMA_URL . '/views/resources/css/settings.css');
	...
	public static function get_header($name = null) {
		...
		if (defined('CMA_GET_HEADER') AND CMA_GET_HEADER) return;
		define('CMA_GET_HEADER', 1);
...
// 			get_header($name);
		...
		if (get_query_var('is_cma_index') AND $title = CMA_Settings::getOption(CMA_Settings::OPTION_INDEX_META_TITLE)) {
			$content = self::replaceHeadTag($content,
				'#(<title>)([^<]+)(</title>)#i',
