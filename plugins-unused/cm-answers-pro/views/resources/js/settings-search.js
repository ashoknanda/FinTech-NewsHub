...
//		console.log(node);
		...
		if (!node) return;
		if (node.parentNode.nodeName == 'SPAN' && node.parentNode.className == 'cma-hl') {
			return;
		}
...
//			console.log(node.textContent);
			var pos = node.textContent.indexOf(text);
			if (pos > -1) {
...
//		console.log(input.val());
		...
		$('#cm-answers-settings-form .cm-settings-row').each(function() {
			var row = $(this);
			if (input.val().length == 0 || this.textContent.indexOf(input.val()) > -1) {
//				console.log(row);
				row.show();
			} else {
				row.hide();
