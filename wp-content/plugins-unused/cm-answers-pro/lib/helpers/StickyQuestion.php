...
class CMA_StickyQuestion
{
    ...
    public static function add_sticky_routines($query)
    {
        ...
        if( $query->get('post_type') == 'cma_thread' )
        {
            ...
            add_filter('posts_groupby', array(get_class(), 'groupby_sticky'));
        }
    }
    ...
    public static function orderby_sticky($original_orderby_statement)
    {
        ...
        return " IFNULL(CAST(cmastickyjoin.meta_value AS UNSIGNED), 0) DESC, " . $original_orderby_statement;
    }
    ...
    public static function join_sticky($wp_join)
    {
        ...
        $wp_join .= " LEFT JOIN $wpdb->postmeta cmastickyjoin ON $wpdb->posts.ID = cmastickyjoin.post_id AND cmastickyjoin.meta_key =  '_sticky_post'";
//         $wp_join .= " LEFT JOIN (
...
// 				ON $wpdb->posts.ID = DD.post_id ";
    ...
    static function groupby_sticky($groupby) {
    	global $wpdb;
    	remove_filter('posts_groupby', array(get_class(), 'groupby_sticky'));
