...
(function ($) {
    ...
    $.extend($.ui, { eotimepicker: { version: "0.3.3"} });

    var PROP_NAME = 'eotimepicker',
        tpuuid = new Date().getTime();

    /* Time picker manager.
    Use the singleton instance of this class, $.eotimepicker, to interact with the time picker.
    Settings for (groups of) time pickers are maintained in an instance object,
    allowing multiple different settings on the same page. */

    function eoTimepicker() {
        ...
        this._defaults = { // Global defaults for all the time picker instances
            ...
            myPosition: 'left top',         // Position of the dialog relative to the input.
                                            // see the position utility for more info : https://jqueryui.com/demos/position/
    ...
    $.extend(eoTimepicker.prototype, {
        ...
        _doKeyDown: function (event) {
            var inst = $.eotimepicker._getInst(event.target);
            var handled = true;
            inst._keyEvent = true;
            if ($.eotimepicker._timepickerShowing) {
                ...
                $.eotimepicker._showTimepicker(this);
            }
            else {
                handled = false;
            }
        ...
        _doKeyUp: function (event) {
            ...
            $.eotimepicker._updateTimepicker(inst);
        },
            ...
            if (showOn == 'button' || showOn == 'both') { // pop-up time picker when 'button' element is clicked
                ...
                $(button).bind("click.eotimepicker", function () {
                    if ($.eotimepicker._timepickerShowing && $.eotimepicker._lastInput == input[0]) {
                        ...
                        $.eotimepicker._showTimepicker(input[0]);
                    }
                    return false;
                });
        ...
        _adjustZIndex: function(input) {
            ...
            var inst = $.eotimepicker._getInst(input);
            inst.tpDiv.css('zIndex', $.eotimepicker._getZIndex(input) +1);
        },
        ...
        _showTimepicker: function (input) {
            ...
            if ($.eotimepicker._isDisabledTimepicker(input) || $.eotimepicker._lastInput == input) { return; } // already here

            // fix v 0.0.8 - close current timepicker before showing another one
            $.eotimepicker._hideTimepicker();

            var inst = $.eotimepicker._getInst(input);
            if ($.eotimepicker._curInst && $.eotimepicker._curInst != inst) {
                $.eotimepicker._curInst.tpDiv.stop(true, true);
            }
            var beforeShow = $.eotimepicker._get(inst, 'beforeShow');
            ...
            if (!$.eotimepicker._pos) { // position below input
                $.eotimepicker._pos = $.eotimepicker._findPos(input);
                $.eotimepicker._pos[1] += input.offsetHeight; // add the height
            }
            var isFixed = false;
            ...
            var offset = { left: $.eotimepicker._pos[0], top: $.eotimepicker._pos[1] };

            $.eotimepicker._pos = null;
            ...
            if ( ( ! inst.inline )  && ( typeof $.ui.position == 'object' ) ) {
                inst.tpDiv.position({
                    of: inst.input,
                    my: $.eotimepicker._get( inst, 'myPosition' ),
                    at: $.eotimepicker._get( inst, 'atPosition' ),
                    // offset: $( "#offset" ).val(),
                    // using: using,
                    collision: 'flip'
                });
                var offset = inst.tpDiv.offset();
                $.eotimepicker._pos = [offset.top, offset.left];
            }
            ...
            offset = $.eotimepicker._checkOffset(inst, offset, isFixed);
            inst.tpDiv.css({ position: ($.eotimepicker._inDialog && $.blockUI ?
			    'static' : (isFixed ? 'fixed' : 'absolute')), display: 'none',
                left: offset.left + 'px', top: offset.top + 'px'
            });
            if ( ! inst.inline ) {
                var showAnim = $.eotimepicker._get(inst, 'showAnim');
                var duration = $.eotimepicker._get(inst, 'duration');

                var postProcess = function () {
                    $.eotimepicker._timepickerShowing = true;
                    var borders = $.eotimepicker._getBorders(inst.tpDiv);
                ...
                $.eotimepicker._adjustZIndex(input);
                //inst.tpDiv.css('zIndex', $.eotimepicker._getZIndex(input) +1);

                if ($.effects && $.effects[showAnim]) {
                    inst.tpDiv.show(showAnim, $.eotimepicker._get(inst, 'showOptions'), duration, postProcess);
                }
                else {
...
*/
        ...
        _checkExternalClick: function (event) {
            if (!$.eotimepicker._curInst) { return; }
            var $target = $(event.target);
            if ($target[0].id != $.eotimepicker._mainDivId &&
				$target.parents('#' + $.eotimepicker._mainDivId).length == 0 &&
				!$target.hasClass($.eotimepicker.markerClassName) &&
				!$target.hasClass($.eotimepicker._triggerClass) &&
				$.eotimepicker._timepickerShowing && !($.eotimepicker._inDialog && $.blockUI))
                $.eotimepicker._hideTimepicker();
        },
        ...
        _hideTimepicker: function (input) {
            ...
            if (this._timepickerShowing) {
                ...
                var postProcess = function () {
                    $.eotimepicker._tidyDialog(inst);
                    this._curInst = null;
                };
                if ($.effects && $.effects[showAnim]) {
                    inst.tpDiv.hide(showAnim, $.eotimepicker._get(inst, 'showOptions'), duration, postProcess);
                }
                else {
        ...
        _optionTimepicker: function(target, name, value) {
            ...
            if (arguments.length == 2 && typeof name == 'string') {
                return (name == 'defaults' ? $.extend({}, $.eotimepicker._defaults) :
        ...
        */
        ...
        selectHours: function (event) {
            ...
            if ( $.eotimepicker._isDisabledTimepicker($target.attr('id')) ) { return false }
        ...
        /* This might look unused but it's called by the $.fn.eotimepicker function with param getTime */
    ...
    $.fn.eotimepicker = function (options) {
        ...
        if ($("#"+$.eotimepicker._mainDivId).length === 0) {
            $('body').append($.eotimepicker.tpDiv);
        }

        var otherArgs = Array.prototype.slice.call(arguments, 1);
        if (typeof options == 'string' && (options == 'getTime' || options == 'getTimeAsDate' || options == 'getHour' || options == 'getMinute' ))
            return $.eotimepicker['_' + options + 'Timepicker'].
			    apply($.eotimepicker, [this[0]].concat(otherArgs));
        if (options == 'option' && arguments.length == 2 && typeof arguments[1] == 'string')
            return $.eotimepicker['_' + options + 'Timepicker'].
                apply($.eotimepicker, [this[0]].concat(otherArgs));
        return this.each(function () {
            typeof options == 'string' ?
			$.eotimepicker['_' + options + 'Timepicker'].
				apply($.eotimepicker, [this].concat(otherArgs)) :
			$.eotimepicker._attachTimepicker(this, options);
        });
    };
    ...
    $.eotimepicker = new eoTimepicker(); // singleton instance
    $.eotimepicker.initialized = false;
    $.eotimepicker.uuid = new Date().getTime();
    $.eotimepicker.version = "0.3.3";
