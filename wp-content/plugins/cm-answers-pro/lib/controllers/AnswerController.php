...
include_once CMA_PATH . '/lib/helpers/ThemeCustomizer.php';
...
//     	add_action('pre_get_posts', array(get_class(), 'registerAdminCustomOrder'), 98, 1);
    	...
    	add_action('cma_display_available_shortcodes', array(__CLASS__, 'cma_display_available_shortcodes'), 1);
    	add_filter('cma_video_helper_process_content', array('CMA_VideoHelper', 'processContent'));

    	CMA_ThemeCustomizer::initialize(array(
		    	'general' => array('referals'),
		    	'appearance' => array_keys(CMA_Settings::getSubcategories('appearance')),
		    	'thread' => array_keys(CMA_Settings::getSubcategories('thread')),
		    	'ads' => array_keys(CMA_Settings::getSubcategories('ads')),
	    	),
	    	array('CMA_Settings', 'getOptionsConfig'), array('CMA_Settings', 'getCategories'), array('CMA_Settings', 'getSubcategories'));
        ...
        add_action('parse_query', array(get_class(), 'processStatusChange'), 10, 1);
        add_filter('wp_nav_menu_items', array(get_class(), 'addMenuItem'), 1, 1);
       	...
       	add_filter('post_class', array(get_class(), 'post_class'), 9999);
        ...
        add_action('cma_referral_link', array(__CLASS__, 'displayReferralLink'));
        self::initThreadPage();
        ...
        if (CMA_Settings::getOption(CMA_Settings::OPTION_SHOW_LOGIN_WIDGET)) {
        	add_action('CMA_login_form', array(get_class(), 'showLoginForm'), 10, 1);
        }
    ...
    static function addDisclaimer($force = false) {
    	if( CMA_Settings::getOption(CMA_Settings::OPTION_DISCLAIMER_ENABLED) ) {
	    	if( $force OR (self::$query->get('post_type') == CMA_Thread::POST_TYPE || self::$query->is_tax(CMA_Category::TAXONOMY) )) {
		        ...
		        wp_localize_script('cma_disclaimer', 'cma_disclaimer_opts', array(
		            'content'    => CMA_Settings::getOption(CMA_Settings::OPTION_DISCLAIMER_CONTENT),
		            'acceptText' => CMA_Labels::getLocalized('disclaimer_accept_btn'),
		            'rejectText' => CMA_Labels::getLocalized('disclaimer_reject_btn')
		        ));
    ...
    public static function getTemplatePart($slug, $name = null) {
    	...
    	switch ($name) {
    		case 'header':
    			if (self::isAjax() OR CMA_Settings::getOption(CMA_Settings::OPTION_THREAD_SHOW_TITLE)) {
    		...
    		case 'resolve-question':
    			...
    			break;
    		case 'meta-question':
    			$avatar = $thread->getAuthorAvatar();
    			$author = sprintf(CMA::__('%s'), $thread->getAuthorLink());
    		...
    		case 'meta-answer':
    			...
    			$author = sprintf(CMA::__('%s'), $answer->getAuthorLink());
				if ( CMA_Settings::getOption(CMA_Settings::OPTION_SHOW_USER_STATS) ) {
    ...
    public static function getUserStats($userId) {
    	...
    	return apply_filters('cma_user_stats', $users[$userId], $userId);
    }
    ...
    public static function registerSidebars() {
        ...
        $sidebarExtra = array_filter(array(
        	'before_widget' => CMA_Settings::getOption(CMA_Settings::OPTION_SIDEBAR_WIDGET_BEFORE),
        	'after_widget' => CMA_Settings::getOption(CMA_Settings::OPTION_SIDEBAR_WIDGET_AFTER),
        	'before_title' => CMA_Settings::getOption(CMA_Settings::OPTION_SIDEBAR_TITLE_BEFORE),
        	'after_title' => CMA_Settings::getOption(CMA_Settings::OPTION_SIDEBAR_TITLE_AFTER),
        ));
    ...
    public static function showLoginForm($options = array())
    {
        echo self::getLoginForm($options);
    }


    public static function getLoginForm($options = array())
    {
    	return self::_loadView('answer/widget/login', $options);
    }
    ...
    public static function processStatusChange(WP_Query $wp_query)
    {
        if( is_admin() && $wp_query->is_main_query() AND $wp_query->get('post_type') == CMA_Thread::POST_TYPE && isset($_REQUEST['cma-action']) )
        {
    ...
    {
    	if (is_archive() AND $widgetOptions = self::restoreWidgetOptions() AND !is_null($widgetOptions['answered'])) {
...
//     	var_dump(get_query_var('post_type'));exit;
            ...
            if ($answerId) {
            	if ($answer = CMA_Answer::getById($answerId)) {
            		if (!empty($_POST['anonymous_nickname'])) {
            			$answer->setAnonymousUserNickname($_POST['anonymous_nickname']);
            		}
            	}
            }
    ...
    protected static function _processAddThread()
    {
        ...
        $data = array(
            ...
            'category'  => $cat,
            'tags'      => $tags,
        	'anonymous_nickname' => (isset($_POST['anonymous_nickname']) ? $_POST['anonymous_nickname'] : null),
        ...
        try
        {
			...
			if (!CMA_Thread::canPostQuestions()) {
				throw new Exception(CMA_Labels::getLocalized('msg_cannot_post_question'));
			}
        	...
        	if (!$error) {
                ...
                if( !$heldForModeration ) {
                	if ($thread->isVisible()) {
                		...
                		$messages[] = CMA_Labels::getLocalized('msg_post_question_success_not_visible');
                	}
                } else {
            ...
            else
            {
                ...
                if( !$heldForModeration ) {
                	if ($thread->isVisible()) {
                		...
                		$msg = CMA_Labels::getLocalized('msg_post_question_success_not_visible');
                	}
                } else {
                	...
                	if ($_POST['cma-redirect'] == '_thread') {
                		if ($heldForModeration) {
                			...
                			if ($thread->isVisible()) {
                				...
                				wp_redirect(isset($_POST['cma-referrer']) ? $_POST['cma-referrer'] : get_post_type_archive_link(CMA_Thread::POST_TYPE), 303);
                			}
                		}
                	} else {
    	...
    	if( !is_user_logged_in() AND !CMA_Settings::getOption(CMA_Settings::OPTION_ALLOW_GUESTS_VOTING)) {
        	...
        	if (!empty($_POST['answerId']) AND $answer = CMA_Answer::getById($_POST['answerId'])) {
        		...
        		if( $answer->getThreadId() == $_POST['threadId'] AND $answer->isVotingAllowed(CMA::getPostingUserId()) ) {
	        		if (!empty($_POST['value']) AND $_POST['value'] == 'up') {
	        			$response['rating'] = $answer->getRating() + 1;
	        			$answer->voteUp();
	        		} else {
	        			$response['rating'] = $answer->getRating() - 1;
	        			$answer->voteDown();
        		...
        		if( $thread->isVotingAllowed(CMA::getPostingUserId()) ) {
        			if (!empty($_POST['value']) AND $_POST['value'] == 'up') {
        				$response['rating'] = $thread->getPostRating() + 1;
        				$thread->voteUp();
        			} else {
        				$response['rating'] = $thread->getPostRating() - 1;
        				$thread->voteDown();
    ...
    public static function adjustBodyClass($wp_classes, $extra_classes)
    {
        ...
        if( in_array('cma_thread', $wp_classes) && (!CMA_Settings::getOption(CMA_Settings::OPTION_SIDEBAR_ENABLED) || !is_active_sidebar('cm-answers-sidebar') ) )
        {
...
// 		var_dump($params);
    ...
    public static function writeCategoriesTableBody($categories, $parentCategoryId = 0, $depth = 0, $atts = array()) {
		if (!empty($categories[$parentCategoryId]) AND is_array($categories[$parentCategoryId])) {
			foreach ($categories[$parentCategoryId] as $category) {
				...
				echo self::_loadView('answer/meta/categories-row', compact('category', 'depth', 'atts'));
				if (!empty($categories[$categoryId])) {
					self::writeCategoriesTableBody($categories, $categoryId, $depth+1, $atts);
				}
			}
		}
	}
	...
	static function post_class($classes) {
		if ( get_query_var('post_type') == CMA_Thread::POST_TYPE || is_tax(CMA_Category::TAXONOMY) || get_query_var('cma_prepared_single') ) {
	...
	static function displayReferralLink() {
		if ( CMA_Settings::getOption(CMA_Settings::OPTION_REFERRAL_ENABLED) AND $code = CMA_Settings::getOption(CMA_Settings::OPTION_AFFILIATE_CODE) ) {
			echo self::_loadView('answer/meta/referral', compact('code'));
		}
	}


	static function cma_display_available_shortcodes() {
