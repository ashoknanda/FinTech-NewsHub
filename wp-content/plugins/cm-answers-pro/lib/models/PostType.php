...
//		$this->unregister_update_hooks();
     ...
     * @return void
     */
    protected function loadPostMeta($meta_key) {
    	// Robert 2015-12-14: don't load all post meta because of the performance issues. Load only cache which already exists.
    	if (isset($this->postMeta[$meta_key])) {
    		$this->postMeta[$meta_key] = get_post_meta($this->getId(), $meta_key, $single = false);
    	}	
...
//         }
    ...
    {
        if($post_id == $this->ID AND $post->post_type == static::POST_TYPE)
        {
            $this->refresh();
        }
    }
    ...
    {
        if (!empty($meta) AND is_array($meta)) foreach($meta as $key => $value)
        {
	...
	public function getPostMeta($meta_key = NULL, $single = TRUE) {
        if ($meta_key !== NULL) { // get a single field
        	// Robert 2015-12-14: dynamically load post meta to the cache when needed:
        	if (!isset($this->postMeta[$meta_key])) {
        		$this->postMeta[$meta_key] = get_post_meta($this->getId(), $meta_key, false);
        	}
            if (!empty($this->postMeta[$meta_key])) {
                if ($single) {
