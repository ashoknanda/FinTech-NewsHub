...
class CMA_Answer {
	...
	const META_ANONYMOUS_USER_NICKNAME = 'cma_anonymous_user_nickname';
	...
	public function saveContent($content) {
		...
		if ($content == $this->comment->comment_content) {
			...
			$rval = wp_update_comment((array)$this->comment);
// 			$rval = $wpdb->update($wpdb->comments, (array)$this->comment, array('comment_ID' => $this->getId()));
    ...
    function setAnonymousUserNickname($val) {
    	update_comment_meta($this->getId(), self::META_ANONYMOUS_USER_NICKNAME, $val);
    }
    
    
    function getAnonymousUserNickname() {
    	$nick = get_comment_meta($this->getId(), self::META_ANONYMOUS_USER_NICKNAME, true);
