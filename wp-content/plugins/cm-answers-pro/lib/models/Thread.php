...
class CMA_Thread extends CMA_PostType {
    ...
    public static $_meta = array(
    	...
    	'anonymousUserNickname'		=> '_cma_anonymous_user_nickname',
    );
...
//            'menu_position' => 4,
        ...
        add_action('generate_rewrite_rules', array(get_class(), 'fixCategorySlugs'), PHP_INT_MAX, 1);
    ...
    public static function fixCategorySlugs($wp_rewrite)
    {
        ...
        $wp_rewrite->rules = array_merge(array(
        	$permalink . '/?$'                   => $wp_rewrite->index . '?post_type=' . self::POST_TYPE,
...
//         $content = self::contentFilter($this->post->post_content, $this->getAuthorId());
    ...
    public function getAuthorLink($simple = false) {
    	if (CMA_Settings::getOption(CMA_Settings::OPTION_ANONYMOUS_USER_ENABLE)) {
    		return $this->getAnonymousUserNickname();
    	}
    	else if ($user = $this->getAuthor()) {
    ...
    public function getLastPosterLink($simple = false) {
    	if ($lastComment = $this->getLastComment()) {
    		return $lastComment->getAuthorLink($simple);
//     		return CMA_Thread::getUser($lastComment->getAuthorId(), $lastComment)->richLink;
    	}
    }
    ...
    public function getCreationDate($format = '')
    {
        if( empty($format) )
        {
        	$format = CMA_Settings::getOption(CMA_Settings::OPTION_DATE_TIME_FORMAT);
        	if (empty($format)) {
...
//         return date_i18n($format, strtotime($dateString));
    ...
    public static function newThread($data = array())
    {
        ...
        else
        {
            ...
            if (!empty($data['anonymous_nickname'])) {
           		$instance->setAnonymousUserNickname($data['anonymous_nickname']);
            }
    ...
    function getAuthorAvatar() {
   		return CMA_Settings::getOption(CMA_Settings::OPTION_SHOW_GRAVATARS) ? get_avatar($this->getAuthorId(), 32) : null;
    }
    
    
    function setAnonymousUserNickname($val) {
    	$this->savePostMeta(array(self::$_meta['anonymousUserNickname'] => $val));
    }
    
    
    function getAnonymousUserNickname() {
    	$nick = $this->getPostMeta(self::$_meta['anonymousUserNickname']);
    	if (empty($nick)) $nick = CMA_Labels::getLocalized('anonymous_user_nickname');
    	return $nick;
    }
