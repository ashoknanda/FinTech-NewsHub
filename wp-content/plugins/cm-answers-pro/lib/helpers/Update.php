...
class CMA_Update {
	...
	protected static $versions = array('2.4.13', '2.5.6', '2.5.9', '2.6.8', '2.9.3', '3.0.0');
	...
	public static function update_comments_types() {
		global $wpdb;
		$ids = $wpdb->get_col("SELECT c.comment_ID FROM $wpdb->comments c
			LEFT JOIN $wpdb->posts p ON p.ID = c.comment_post_ID
			WHERE p.post_type = 'cma_thread'
				AND c.comment_type = ''");
		foreach ($ids as $id) {
			$wpdb->update($wpdb->comments, array('comment_type' => 'cma_answer'), array('comment_ID' => $id));
		}
	}
	
	

	public static function update_3_0_0() {
		// Move sidebar settings to a separate options
		$sidebar = get_option('CMA_sidebar_settings');
		if (isset($sidebar['before_widget'])) {
			update_option(CMA_Settings::OPTION_SIDEBAR_WIDGET_BEFORE, $sidebar['before_widget']);
		}
		if (isset($sidebar['before_title'])) {
			update_option(CMA_Settings::OPTION_SIDEBAR_TITLE_BEFORE, $sidebar['before_title']);
		}
		if (isset($sidebar['after_title'])) {
			update_option(CMA_Settings::OPTION_SIDEBAR_TITLE_AFTER, $sidebar['after_title']);
		}
		if (isset($sidebar['after_widget'])) {
			update_option(CMA_Settings::OPTION_SIDEBAR_WIDGET_AFTER, $sidebar['after_widget']);
		}
	}
